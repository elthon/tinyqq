...
var HttpService = require("./service");

function TinyQQ(auth) {
	...
	//启动HttpService服务，用于提供管理控制台。
	this.httpServer = new HttpService(this);
	this.login();
}

TinyQQ.prototype.getClient = function(){
	return this.libqq;
}

TinyQQ.prototype.isMe = function(uin) {
	if (this.account.uin === uin) {
		return true;
	}
	return false;
}
...
TinyQQ.prototype.login = function() {
	...
	then(function(defer) {
		...
		console.log("开始获取登录用户的信息、群信息、好友信息...");

		then.parallel([libqq.fetchMyInfo, libqq.fetchMyBuddies, _.partial(libqq.fetchMyGroupList, true)], libqq,true).
		then(function(d,value) {
			console.log("开始加载动态插件...");
