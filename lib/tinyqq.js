...
function login(u, p) {
	...
	then(function(defer) {
		...
		libqq.on('groupmessage', function(msg) {
			...
			var group = account.qun[msg.from_uin];
			if (group.name.match(/antvision/g)) {
				return;
			}
					...
					then(function(d3) {
						...
						request.get(opt, function(error, res, body) {
							...
							_.each(body.statuses, function(status) {
								weibo.push(status.text);
							})

							d3(null, weibo);
						})
					}).then(function(d3, weibo) {
						console.log(weibo[0]);
						sendMsg(gid, weibo[0], true);
					}, function(d3, error) {
...
function sendMsg(uin, input, group) {
